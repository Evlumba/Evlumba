...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'profiles_blog_header_image_url_max_len'
      and conrelid = 'public.profiles'::regclass
  ) then
    alter table public.profiles
      add constraint profiles_blog_header_image_url_max_len
      check (
        coalesce(char_length(coalesce(about_details->>'blogHeaderImageUrl', '')), 0) <= 3000000
      );
  end if;
end
$$;
...
create or replace function public.can_join_forum(user_uuid uuid)
returns boolean
language plpgsql
stable
security definer
...
begin
  ...
  profile_role := public.get_profile_role(user_uuid);
  if profile_role in ('designer', 'designer_pending') then
    return true;
  end if;
...
grant execute on function public.can_join_forum(uuid) to authenticated;
...
create index if not exists blog_posts_author_created_idx
on public.blog_posts (author_id, created_at desc);

create index if not exists blog_post_likes_user_idx
on public.blog_post_likes (user_id, created_at desc);
...
drop trigger if exists blog_post_comments_touch_updated_at on public.blog_post_comments;
...
create trigger blog_post_comments_touch_updated_at
...
declare
  ...
  resolved_role text;
begin
  resolved_role := nullif(btrim(coalesce(new.role, old.role, 'homeowner')), '');
  if resolved_role is null then
    resolved_role := 'homeowner';
  end if;
...
drop policy if exists "blog_posts: public read published or own" on public.blog_posts;
drop policy if exists "blog_posts: professionals can insert own" on public.blog_posts;
drop policy if exists "blog_posts: author can update own" on public.blog_posts;
drop policy if exists "blog_posts: author can delete own" on public.blog_posts;
drop policy if exists "blog_post_likes: public read" on public.blog_post_likes;
drop policy if exists "blog_post_likes: user can insert own" on public.blog_post_likes;
drop policy if exists "blog_post_likes: user can delete own" on public.blog_post_likes;
drop policy if exists "blog_post_comments: public read" on public.blog_post_comments;
drop policy if exists "blog_post_comments: user can insert own" on public.blog_post_comments;
drop policy if exists "blog_post_comments: user can update own" on public.blog_post_comments;
drop policy if exists "blog_post_comments: user can delete own" on public.blog_post_comments;
...
with check (
  auth.uid() = user_id
  and public.can_join_forum(auth.uid())
...
create policy "blog_posts: public read published or own"
...
create policy "blog_posts: professionals can insert own"
...
with check (
  ...
  and char_length(btrim(title)) between 5 and 220
  and char_length(btrim(content)) between 20 and 50000
  and char_length(btrim(slug)) between 3 and 240
);

create policy "blog_posts: author can update own"
...
create policy "blog_posts: author can delete own"
on public.blog_posts for delete
to authenticated
using (auth.uid() = author_id);

create policy "blog_post_likes: public read"
on public.blog_post_likes for select
to public
using (true);

create policy "blog_post_likes: user can insert own"
...
create policy "blog_post_likes: user can delete own"
on public.blog_post_likes for delete
to authenticated
using (auth.uid() = user_id);

create policy "blog_post_comments: public read"
on public.blog_post_comments for select
to public
using (true);

create policy "blog_post_comments: user can insert own"
...
create policy "blog_post_comments: user can update own"
...
create policy "blog_post_comments: user can delete own"
...
grant select on table public.blog_posts, public.blog_post_likes, public.blog_post_comments to anon, authenticated;
grant insert, update, delete on table public.blog_posts to authenticated;
grant insert, delete on table public.blog_post_likes to authenticated;
grant insert, update, delete on table public.blog_post_comments to authenticated;
...
create index if not exists user_moderation_states_ban_idx
on public.user_moderation_states (is_banned, banned_until);

create index if not exists user_moderation_states_deleted_idx
on public.user_moderation_states (is_deleted, deleted_at);
...
create or replace function public.touch_user_moderation_updated_at()
returns trigger
language plpgsql
set search_path = public
as $$
...
drop trigger if exists user_moderation_states_touch_updated_at on public.user_moderation_states;
...
create trigger user_moderation_states_touch_updated_at
before update on public.user_moderation_states
for each row
execute function public.touch_user_moderation_updated_at();
...
create or replace function public.is_admin(user_uuid uuid, required_role text default 'admin')
returns boolean
language sql
stable
security definer
...
as $$
  select case
    when required_role = 'super_admin' then public.get_admin_role(user_uuid) = 'super_admin'
    when required_role = 'admin' then public.get_admin_role(user_uuid) in ('admin', 'super_admin')
    else false
  end;
$$;
...
drop policy if exists "admin_users: admins can read" on public.admin_users;
drop policy if exists "admin_users: super admins can insert" on public.admin_users;
drop policy if exists "admin_users: super admins can update" on public.admin_users;
drop policy if exists "admin_users: super admins can delete" on public.admin_users;
drop policy if exists "user_moderation_states: admins can read" on public.user_moderation_states;
drop policy if exists "user_moderation_states: admins can upsert" on public.user_moderation_states;
drop policy if exists "admin_audit_logs: admins can read" on public.admin_audit_logs;
drop policy if exists "admin_audit_logs: admins can insert own logs" on public.admin_audit_logs;

create policy "admin_users: admins can read"
on public.admin_users for select
to authenticated
using (public.is_admin(auth.uid(), 'admin'));

create policy "admin_users: super admins can insert"
on public.admin_users for insert
to authenticated
with check (public.is_admin(auth.uid(), 'super_admin'));

create policy "admin_users: super admins can update"
on public.admin_users for update
to authenticated
using (public.is_admin(auth.uid(), 'super_admin'))
with check (public.is_admin(auth.uid(), 'super_admin'));

create policy "admin_users: super admins can delete"
on public.admin_users for delete
to authenticated
using (public.is_admin(auth.uid(), 'super_admin'));

create policy "user_moderation_states: admins can read"
on public.user_moderation_states for select
to authenticated
using (public.is_admin(auth.uid(), 'admin'));

create policy "user_moderation_states: admins can upsert"
on public.user_moderation_states for all
to authenticated
using (public.is_admin(auth.uid(), 'admin'))
with check (public.is_admin(auth.uid(), 'admin'));

create policy "admin_audit_logs: admins can read"
on public.admin_audit_logs for select
to authenticated
using (public.is_admin(auth.uid(), 'admin'));

create policy "admin_audit_logs: admins can insert own logs"
on public.admin_audit_logs for insert
to authenticated
with check (
  public.is_admin(auth.uid(), 'admin')
  and actor_user_id = auth.uid()
);

grant select, insert, update, delete on table public.admin_users to authenticated;
grant select, insert, update on table public.user_moderation_states to authenticated;
