...
export default function BlogDetailClient({
  ...
  initialData: BlogDetailInitialData;
}) {
  ...
  const [profilesById, setProfilesById] = useState<Record<string, ProfileBrief>>(
    initialData.profilesById
  );
  const [adminRoleByUserId, setAdminRoleByUserId] = useState<Record<string, AdminRole>>(
    initialData.adminRoleByUserId
  );
  const [comments, setComments] = useState<BlogCommentRow[]>(initialData.comments);
  ...
  const toggleLike = async () => {
    if (!ensureLoggedInForInteraction() || !userId || likeLoading) return;
    setLikeLoading(true);
    setError(null);
  ...
  const submitComment = async () => {
    if (!ensureLoggedInForInteraction() || !userId || commentSending) return;
    ...
    const nextAdminRoles = await fetchUserAdminRoles(
      nextCommentRows.map((comment) => comment.user_id)
    );
    ...
    if (missingUserIds.length > 0) {
      const { data: profileRows } = await supabase.rpc("get_profile_briefs", {
    ...
    <main className="mx-auto w-full max-w-4xl py-4">
      ...
      <article className="mt-3 overflow-hidden rounded-3xl border border-black/10 bg-white shadow-[0_24px_60px_-45px_rgba(0,0,0,0.35)]">
        ...
        <div className="p-6 md:p-8">
          ...
          {post.excerpt?.trim() ? (
            <p className="mt-3 text-base leading-7 text-slate-700">{post.excerpt.trim()}</p>
          ) : null}
        ...
        {comments.length === 0 ? (
          ...
          <div className="mt-3 space-y-3">
            {comments.map((comment) => {
              ...
              const commentAuthorAdminBadge = getAdminBadgeLabel(
                adminRoleByUserId[comment.user_id]
              );
              return (
                <article
                  key={comment.id}
