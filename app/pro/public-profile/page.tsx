...
type PublicReview = {
  ...
  created_at: string;
  author_name: string;
  author_admin_role: "admin" | "super_admin" | null;
};
...
function getAdminBadgeLabel(role: "admin" | "super_admin" | null | undefined) {
                ...
                {reviews.length === 0 ? (
                  ...
                  <div className="mt-3 space-y-2">
                    {reviews.slice(0, 6).map((review) => {
                      const adminBadgeLabel = getAdminBadgeLabel(review.author_admin_role);
                      return (
                        <article key={review.id} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                          <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center gap-1.5">
                              ...
                              {adminBadgeLabel ? (
                                <span className="rounded-full border border-sky-300 bg-sky-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-sky-800">
                                  {adminBadgeLabel}
                                </span>
                              ) : null}
                            </div>
