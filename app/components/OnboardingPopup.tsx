...
function getImpressionCount(flowId: string): number {
  try {
    return Number(localStorage.getItem(`evlumba:onboarding:${flowId}`) || "0");
  } catch {
    return 0;
  }
}
...
function incrementImpression(flowId: string) {
  try {
    const current = getImpressionCount(flowId);
    localStorage.setItem(`evlumba:onboarding:${flowId}`, String(current + 1));
  } catch {}
}
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const res = await fetch(`/api/public/onboarding?role=${mappedRole}`);
    ...
    >
      ...
      >
        ...
        <button
          type="button"
          onClick={close}
          className="absolute right-4 top-4 z-10 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-black/5 text-slate-500 hover:bg-black/10 transition"
        >
          ...
          <div className="mt-6 flex items-center justify-between gap-3">
            {!isFirst ? (
              <button
                type="button"
                onClick={prev}
                className="cursor-pointer rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition"
              >
                Geri
              </button>
            ) : (
              <button
                type="button"
                onClick={close}
                className="cursor-pointer text-sm text-slate-400 hover:text-slate-600 transition"
              >
                Atla
              </button>
            )}
            ...
            <button
              type="button"
              onClick={next}
              className="cursor-pointer rounded-xl bg-gradient-to-b from-[#1a5c3a] to-[#145230] px-6 py-2.5 text-sm font-bold text-white shadow-[0_4px_12px_-4px_rgba(26,92,58,0.4)] hover:from-[#1e6b44] hover:to-[#185d39] transition"
            >
              {isLast ? "Başla!" : "İleri"}
            </button>
          </div>
