...
function buildInstagramProfileUrl(value: string) {
...
async function onPickCover(file: File | null) {
  ...
  function connectInstagram() {
    if (typeof window === "undefined") return;
    const input = window.prompt("Instagram kullanıcı adı veya profil linki gir");
    if (input === null) return;

    const normalized = normalizeInstagramHandle(input);
    if (!normalized) {
      toast("Geçerli bir Instagram kullanıcı adı veya profil linki gir.");
      return;
    }

    setDraft((prev) => ({ ...prev, instagram: normalized }));
    setActiveTab("contact");
    toast(`Instagram bağlandı: @${normalized}`);
  }
  ...
  const instagramProfileUrl = buildInstagramProfileUrl(draft.instagram);
  const instagramHandlePreview = normalizeInstagramHandle(draft.instagram);
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                <div className="grid gap-2 md:grid-cols-[1fr_auto_auto]">
                  <input
                    ...
                    placeholder="Instagram kullanıcı adı veya profil linki"
                  />
                  <button
                    type="button"
                    onClick={connectInstagram}
                    className="rounded-xl border border-pink-200 bg-pink-50 px-3 py-2 text-sm font-semibold text-pink-700 hover:bg-pink-100"
                  >
                    Instagram Bağla
                  </button>
                  {instagramProfileUrl ? (
                ...
                <p className="mt-2 text-xs text-slate-500">
                  {instagramHandlePreview
                    ? `Bağlı hesap: @${instagramHandlePreview}. Profesyonel profilinde son 5 paylaşım slider olarak gösterilir.`
                    : "Bağlandığında profesyonel profilinde son 5 Instagram paylaşımı slider olarak görünür."}
                </p>
              </div>
