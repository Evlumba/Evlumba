...
const MAX_PROJECT_IMAGE_MB = 5;
const MAX_PROJECT_IMAGE_BYTES = MAX_PROJECT_IMAGE_MB * 1024 * 1024;
const ROOM_OPTIONS = [
...
async function validateAndReadImage(file: File) {
  if (file.size > MAX_PROJECT_IMAGE_BYTES) {
    ...
    throw new Error(
      `Görsel en fazla ${MAX_PROJECT_IMAGE_MB} MB olabilir. Seçilen dosya: ${selectedMb} MB.`
    );
  }
...
export default function DesignerProjectsPage() {
  ...
  async function onPickCover(file: File | null) {
    ...
    try {
      ...
      setSubmitMessage({ type: "success", text: "Kapak görseli eklendi." });
    } catch (error) {
  ...
  async function onPickGallery(files: FileList | null) {
    ...
    if (remaining <= 0) {
      setSubmitMessage({ type: "error", text: `En fazla ${MAX_GALLERY_IMAGES} görsel ekleyebilirsin.` });
      return;
    }
    ...
    if (added.length) {
      ...
      setSubmitMessage({ type: "success", text: `${added.length} görsel eklendi.` });
    }
  }
